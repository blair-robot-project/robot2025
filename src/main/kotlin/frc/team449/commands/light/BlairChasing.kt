...
class BlairChasing(private val led: Light) : Command() {
    ...
    override fun execute() {
        for (i in LightConstants.SECTION_1_START..LightConstants.SECTION_1_END) {
            // This number is related to how many lights will show up between the high and low
            // intensity (which technically also affects how fast itll cycle)
            val saturation =
                MathUtil.inputModulus(firstSaturation1 + i * 200.0 / led.buffer.length, 0.0, 255.0)
        ...
        for (i in LightConstants.SECTION_2_START..LightConstants.SECTION_2_END) {
            ...
            val saturation =
                MathUtil.inputModulus(
                    firstSaturation2 +
                        (i - LightConstants.SECTION_2_START) * 200.0 / led.buffer.length,
                    0.0,
                    255.0,
                )
            led.setHSV(i, 0, saturation.toInt(), 255)
