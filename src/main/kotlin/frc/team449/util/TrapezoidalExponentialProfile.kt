...
//  and figure out to do for current vel > 0 situations (ideas: don't do anything, or use a trap
// profile in that case idk)

/**
 * @param effectiveGearing is motor rotations : output rotations (should be greater than 1 if a
 *   reduction
...
class TrapezoidalExponentialProfile(
    ...
    private fun evaluateExp(
        ...
        return B.pow(2) * exp(2 * C * t) + (D * B / C + 2 * A * B) * exp(C * t) + A * D * t -
            D * B / C -
            deltaX * D + A.pow(2)
    }
    ...
    private val vLim =
        pulleyRadius * freeSpeed / effectiveGearing *
            (1 - (currentLimit - freeCurrent) / (stallCurrent - freeCurrent))
    private val effectiveStallTorque = stallTorque * numMotors
    private val effectiveGravity = 9.81 * sin(Units.degreesToRadians(angle))
    private var aLim =
        ((currentLimit - freeCurrent) / (stallCurrent - freeCurrent) *
            effectiveStallTorque *
            effectiveGearing) / (systemMass * pulleyRadius) - effectiveGravity
    private val vFree =
        pulleyRadius * freeSpeed / effectiveGearing *
            (1 -
                systemMass * effectiveGravity * pulleyRadius /
                    effectiveGearing /
                    effectiveStallTorque)
    ...
    private val dt23 =
        if ((vFree - vMax2) / (vFree - vLim) > 0)
            -(vFree - vLim) / aLim * ln((vFree - vMax2) / (vFree - vLim))
        else Double.NaN
    private val dt24 =
        if (ENTER_EXP) expDecelIntercept(vFree, vLim, aLim, aStop, finalDistance, x20)
        else Double.NaN
    private val t2f =
        t20 +
            if (!dt23.isNaN() && !dt24.isNaN()) min(dt23, dt24)
            else if (!dt24.isNaN()) dt24 else 0.0
    private val x2f =
        x20 +
            vFree * (t2f - t20) +
            (vFree - vLim).pow(2) / aLim * (exp(-aLim / (vFree - vLim) * (t2f - t20)) - 1)
    ...
    private val dt34 =
        if (!x30.isNaN()) (finalDistance - x30) - vMax2 - vMax2 / 2 / aStop else Double.NaN
    ...
    private fun sample1(t: Double): Pair<Double, Double> {
        return if (switchedStartingAndFinal) {
            Pair(trueStartingDistance - (0.5 * aLim * t.pow(2)), -(aLim * t))
        } else {
            Pair(0.5 * aLim * t.pow(2) + trueStartingDistance, aLim * t)
        }
    }
    ...
    private fun sample2(t: Double): Pair<Double, Double> {
        return if (switchedStartingAndFinal) {
            Pair(
                trueStartingDistance -
                    (x20 +
                        vFree * (t - t20) +
                        (vFree - vLim).pow(2) / aLim *
                            (exp(-aLim / (vFree - vLim) * (t - t20)) - 1)),
            ...
            Pair(
                x20 +
                    vFree * (t - t20) +
                    (vFree - vLim).pow(2) / aLim * (exp(-aLim / (vFree - vLim) * (t - t20)) - 1) +
                    trueStartingDistance,
                vFree - (vFree - vLim) * exp(-aLim / (vFree - vLim) * (t - t20)),
            )
        }
    ...
    private fun sample3(t: Double): Pair<Double, Double> {
        return if (switchedStartingAndFinal) {
            Pair(
                // hey, this was vMax in this script. Shouldn't this be vMax2 for the actual top
                // speed?
    ...
    fun calculate(t: Double): Pair<Double, Double> {
        if (abs(trueFinalDistance - trueStartingDistance) < tolerance)
            return Pair(trueFinalDistance, 0.0)
