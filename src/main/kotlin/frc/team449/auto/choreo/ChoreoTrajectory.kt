...
/**
 * Data structure for parsed Choreo Trajectory, and interpolates between points given in the Choreo
 * file. Use the parser function below to create a ChoreoTrajectory.
 *
...
class ChoreoTrajectory(
    ...
    fun initialPose(): Pose2d {
        ...
        return Pose2d(initialState[0, 0], initialState[0, 1], Rotation2d(initialState[0, 2]))
    }
    ...
    companion object {
        ...
        /**
         * @param filenames Filenames of .traj document file. By default, it will check in the
         *   deploy/choreo folder
         */
            ...
            for (filename in filenames) {
                val path =
                    Filesystem.getDeployDirectory()
                        .absolutePath
                        .plus("/choreo/$subFolder/$filename.traj")
                val document =
                    (JSONParser().parse(FileReader(File(path).absolutePath)) as JSONObject)[
                        "trajectory"]
                        as JSONObject
                ...
                trajList.add(ChoreoTrajectory(filename, info.first, totalTime, info.second))
            }
        ...
        private fun parse(
            trajectory: JSONArray
        ): Pair<InterpolatingMatrixTreeMap<Double, N2, N3>, ArrayList<Double>> {
            val stateMap = InterpolatingMatrixTreeMap<Double, N2, N3>()
