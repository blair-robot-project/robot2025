...
fun createSparkMax(
    ...
    config
        ...
        .smartCurrentLimit(currentLimit.`in`(Amps).toInt())
        .voltageCompensation(RobotController.getBatteryVoltage())

    config.encoder.positionConversionFactor(upr * gearing).velocityConversionFactor(upr * gearing)
...
fun createFollowerSpark(id: Int, leader: SparkMax, invertedFromLeader: Boolean): SparkMax {
    ...
    config
        ...
        .smartCurrentLimit(leaderConfig.smartCurrentLimit, leaderConfig.smartCurrentFreeLimit)
...
fun createKraken(
    ...
    torqueCurrentLimit: Current = Amps.of(60.0),
    kS: Double = 0.0,
    kV: Double = 0.0,
    kA: Double = 0.0,
    kG: Double = 0.0,
    ...
    config.MotorOutput.NeutralMode =
        if (brakeMode) NeutralModeValue.Brake else NeutralModeValue.Coast
