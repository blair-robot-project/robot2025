...
/**
 * This class uses an absolute encoder, gear ratio and UPR to give the absolute position of the
 * module or rotational velocity of the module.
 *
...
open class AbsoluteEncoder(
    ...
    override fun getPositionNative(): Double {
        return if (inverted) {
            filter.calculate(MathUtil.inputModulus(1 - (enc.get() - offset), min, max))
        } else {
            filter.calculate(MathUtil.inputModulus((enc.get() - offset), min, max))
        }
    }
    ...
    companion object {
         ...
         * @param unitPerRotation units measured when done one rotation (e.g 360 degrees per
         *   rotation)
        ...
        fun <T : MotorController> creator(
            ...
            max: Double = 0.5,
            min: Double = -0.5,
        ): EncoderCreator<T> = EncoderCreator { name, _, _ ->
            val enc =
                AbsoluteEncoder(
                    name,
