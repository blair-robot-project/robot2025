...
object VisionConstants {
    ...
    private val TEST_TAG_LAYOUT =
        AprilTagFieldLayout(listOf(AprilTag(3, Pose3d())), 16.4846, 8.1026)

    //  val TAG_LAYOUT: AprilTagFieldLayout = TEST_TAG_LAYOUT

    /** WPILib's AprilTagFieldLayout for the 2024 Crescendo Game */
    val TAG_LAYOUT: AprilTagFieldLayout =
        AprilTagFieldLayout(Filesystem.getDeployDirectory().absolutePath + "/modded_layout.json")

    /** Robot to Camera distance */
    val backLeft =
        Transform3d(
            Translation3d(
                Units.inchesToMeters(-10.696),
                Units.inchesToMeters(10.848),
                Units.inchesToMeters(9.11),
            ),
    ...
    val TAG_MODEL = TargetModel(Units.inchesToMeters(6.5), Units.inchesToMeters(6.5))
    ...
    val VISION_SIM = VisionSystemSim("main")
    ...
    val ESTIMATORS: ArrayList<ApriltagCamera> =
        arrayListOf(
            //    ApriltagCamera(
            //      "back_left",
            //      TAG_LAYOUT,
