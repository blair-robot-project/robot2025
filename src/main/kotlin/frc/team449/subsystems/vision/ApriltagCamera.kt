...
class ApriltagCamera(
    ...
    init {
        if (RobotBase.isSimulation()) {
            ...
            cameraProp.setCalibration(
                ...
                Rotation2d.fromDegrees(VisionConstants.SIM_FOV_DEG),
            )
            cameraProp.setCalibError(
                VisionConstants.SIM_CALIB_AVG_ERR_PX,
                VisionConstants.SIM_CALIB_ERR_STDDEV_PX,
            )
        ...
        for (result in results) {
            ...
            if (RobotBase.isSimulation()) {
                visionEst.ifPresentOrElse({ est ->
                    getSimDebugField()!!.getObject("VisionEstimation").pose =
    ...
    fun getEstimationStdDevs(numTags: Int, avgDist: Double): Matrix<N3, N1> {
        ...
        return estStdDevs.times(1 + avgDist.pow(VisionConstants.ORDER) * VisionConstants.PROPORTION)
    }
