...
/**
 * Create an OI for controlling a holonomic drivetrain (probably swerve). The x and y axes on one
 * joystick are used to control x and y velocity (m/s), while the x axis on another joystick is used
 * to control rotational velocity (m/s).
 *
 * <p> The magnitude of the acceleration is clamped
 *
 * <p>Note that the joystick's X axis corresponds to the robot's/field's Y and vice versa
 *
 ...
 * @param fieldOriented Whether the OI x and y translation should be relative to the field rather
 *   than relative to the robot. This better be true.
 */
class HolonomicOI(
    ...
    /** @return The [ChassisSpeeds] for the given x, y and rotation input from the joystick */
    override fun execute() {
        ...
        if (this.fieldOriented()) {
            drive.set(ChassisSpeeds.fromFieldRelativeSpeeds(vel.x, vel.y, rotScaled, drive.heading))
        } else {
    ...
    companion object {
        fun createHolonomicOI(drive: HolonomicDrive, driveController: XboxController): HolonomicOI {
            return HolonomicOI(
                drive,
                {
                    if (abs(driveController.leftY) < RobotConstants.DRIVE_RADIUS_DEADBAND) .0
                    else -driveController.leftY
                },
                {
                    if (abs(driveController.leftX) < RobotConstants.DRIVE_RADIUS_DEADBAND) .0
                    else -driveController.leftX
                },
                {
                    if (abs(driveController.getRawAxis(4)) < RobotConstants.ROTATION_DEADBAND) .0
                    else -driveController.getRawAxis(4)
                },
                SlewRateLimiter(
