...
/** Helper class to create OIs for a differential drivetrain (arcade, curvature, or tank) */
object DifferentialOIs {
    /**
     * Create an OI for arcade drive. One throttle controls forward-backward speed, the other;
     * controls rotation.
     *
    ...
    /**
     * Create OI for curvature drive (drives like a car). One throttle controls forward-backward
     * speed, like arcade, but the other controls curvature instead of rotation. Ramping is still
     * applied to rotation instead of curvature.
     *
     * @param drive The drivetrain
     * @param xThrottle Throttle to get forward-backward movement
     * @param rotThrottle Throttle to get rotation
     * @param xRamp Used for limiting forward-backward acceleration
     * @param rotRamp Used for limiting rotational acceleration
     * @param turnInPlace When this returns true, turns in place instead of turning like a car
     */
    fun createCurvature(
        drive: DifferentialDrive,
        xThrottle: () -> Double,
    ...
    /**
     * Create an OI for tank drive. Each throttle controls one side of the drive separately. Each
     * side is also ramped separately.
     *
     * <p>
    ...
    /**
     * Scales differential drive throttles from [-1, 1] to [-maxSpeed, maxSpeed], then applies
     * ramping to give the final [ChassisSpeeds].
     *
     * <p>
     * Do note that although this is given a [DifferentialDriveWheelSpeeds] object, the ramping
     * isn't applied to the left and right side but to the linear and rotational velocity using a
     * [ChassisSpeeds] object.
     *
     * @param wheelThrottles The left and right wheel throttles
     * @param kinematics Kinematics object used for turning differential drive wheel speeds to
     *   chassis speeds
    ...
    private fun scaleAndApplyRamping(
        ...
        val chassisSpeeds =
            kinematics.toChassisSpeeds(DifferentialDriveWheelSpeeds(leftVel, rightVel))
