...
class SwerveOrthogonalCommand(
    ...
    private val allianceCompensation = {
        if (DriverStation.getAlliance().getOrNull() == DriverStation.Alliance.Red) PI else 0.0
    }
    private val directionCompensation = {
        if (DriverStation.getAlliance().getOrNull() == DriverStation.Alliance.Red) -1.0 else 1.0
    }
    ...
    private val rotCtrl =
        PIDController(RobotConstants.SNAP_KP, RobotConstants.SNAP_KI, RobotConstants.SNAP_KD)
    ...
    /**
     * Just a helper command factory to point at a given angle and stop the heading lock once you
     * get into tolerance If you want to customize when the heading lock is lifted, use the internal
     * snapToAngle, checkSnapToAngleTolerance, and exitSnapToAngle functions
     */
    ...
    override fun execute() {
        ...
        if (RobotConstants.USE_ACCEL_LIMIT) {
            ...
            magAccClamped =
                MathUtil.clamp(magAcc, -RobotConstants.MAX_ACCEL, RobotConstants.MAX_ACCEL)
        ...
        if (fieldOriented.invoke()) {
            /**
             * Quick fix for the velocity skewing towards the direction of rotation by rotating it
             * with offset proportional to how much we are rotating
             */
            ...
            drive.set(ChassisSpeeds(vel.x, vel.y, rotScaled))
        }
    }
    ...
    override fun initSendable(builder: SendableBuilder) {
        ...
        builder.addDoubleProperty(
            "1.1 currX",
            {
                if (abs(controller.leftY) < RobotConstants.DRIVE_RADIUS_DEADBAND) .0
                else -controller.leftY
            },
            null,
        )
        builder.addDoubleProperty(
            "1.2 currY",
            {
                if (abs(controller.leftX) < RobotConstants.DRIVE_RADIUS_DEADBAND) .0
                else -controller.leftX
            },
            null,
        )
        ...
        builder.addBooleanProperty(
            "6.2 In Snap to Angle Tolerance",
            ::checkSnapToAngleTolerance,
            null,
        )
    }
}
