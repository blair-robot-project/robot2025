 ...
 * @param location The location of the module in reference to the center of the robot. NOTE: In
 *   relation to the robot [+X is forward, +Y is left, and +THETA is Counter Clock-Wise].
 */
open class SwerveModuleNEO(
    ...
    override val desiredState = SwerveModuleState(0.0, Rotation2d())
    ...
    override fun update() {
        ...
        val drivePid = driveController.calculate(drivingMotor.encoder.velocity)
        val driveFF = driveFeedforward.calculate(desiredState.speedMetersPerSecond)

        drivingMotor.setVoltage(drivePid + driveFF)

        /** CONTROL direction of module */
        val turnPid = turnController.calculate(turningMotor.encoder.position)

        turningMotor.set(
            turnPid +
                sign(desiredState.angle.radians - turnEncoder.position) * SwerveConstants.STEER_KS
        )
    }
...
class SwerveModuleSimNEO(
    ...
    private val driveEncoder =
        Encoder.SimController(
            NEOEncoder.creator(SwerveConstants.DRIVE_UPR, SwerveConstants.DRIVE_GEARING)
                .create("drive encoder", drivingMotor, true)
        )
    private var prevTime = Timer.getFPGATimestamp()
    override var state: SwerveModuleState
        get() = SwerveModuleState(driveEncoder.velocity, Rotation2d(turningMotorEncoder.position))
        set(desiredState) {
    ...
    override val position: SwerveModulePosition
        get() =
            SwerveModulePosition(driveEncoder.position, Rotation2d(turningMotorEncoder.position))
