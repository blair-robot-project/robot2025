...
class LengthPivotSim(
  ...
  fun setState(angleRadians: Double, velocityRadPerSec: Double) {
    this.setState(VecBuilder.fill(MathUtil.clamp(angleRadians, this.m_minAngle, this.m_maxAngle), velocityRadPerSec))
  }
  ...
  override fun updateX(currentXhat: Matrix<N2?, N1>, u: Matrix<N1?, N1>, dtSeconds: Double): Matrix<N2?, N1> {
    val updatedXhat = NumericalIntegration.rkdp({ x: Matrix<N2?, N1>, _u: Matrix<N1?, N1>? ->
    ...
    return if (this.wouldHitLowerLimit(updatedXhat[0, 0])) {
      ...
      (if (this.wouldHitUpperLimit(updatedXhat[0, 0])) VecBuilder.fill(this.m_maxAngle, 0.0) else updatedXhat) as Matrix<N2?, N1>
    }
  }
